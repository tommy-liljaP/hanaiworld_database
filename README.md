...
### Excelファイルだけ更新する場合（おすすめ）

`はないわーるど.xlsx` のみ変更した場合は、GitHubのWeb画面から直接更新できます。

1. GitHubでこのリポジトリを開く
2. `はないわーるど.xlsx` を開く
3. **…（または Add file → Upload files）** から新しいファイルをアップロードして上書き
4. Commit changes を押す

数十秒～数分後にCloudflare Pagesが自動で再デプロイされ、サイトにも反映されます。

---

### HTML・CSS・JavaScriptも更新する場合

`index.html`、`style.css`、`app.js`、`vendor`フォルダなどを変更した場合は、Gitで更新してください。

PowerShellを開き、プロジェクトフォルダへ移動します。

```powershell
cd "C:\Users\tommy\OneDrive\ドキュメント\はないわーるどデータベース関連ファイル"
```
